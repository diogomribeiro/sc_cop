...
inputFile = "../data/functional_enrichment/cuomo2021_functional_enrichment.tsv"
...
dataset[ItemGroup == "1: bulk 15 PCA"]$ItemGroup = "BulkCOPs"
dataset[ItemGroup == "2: union"]$ItemGroup = "scCOPs"
dataset[ItemGroup == "3: intersect 5"]$ItemGroup = "scCOPs>=5"
...
g1 = ggplot( ) +
  ...
  geom_segment(data = dataset[ItemGroup == "BulkCOPs"], aes(x = confmin, xend = confmax, y = ExternalList, yend = ExternalList, color = ItemGroup), position = position_nudge(y = 0.2), size = 3 ) +
  geom_point(data = dataset[ItemGroup == "BulkCOPs"], aes(x = odds, y = ExternalList, fill = ItemGroup), position = position_nudge(y = 0.2), size = 6, shape = 21) + 
  geom_segment(data = dataset[ItemGroup == "scCOPs"], aes(x = confmin, xend = confmax, y = ExternalList, yend = ExternalList, color = ItemGroup), position = position_nudge(y = 0.0), size = 3 ) +
  geom_point(data = dataset[ItemGroup == "scCOPs"], aes(x = odds, y = ExternalList, fill = ItemGroup), position = position_nudge(y = 0.0), size = 6, shape = 24) + 
  geom_segment(data = dataset[ItemGroup == "scCOPs>=5"], aes(x = confmin, xend = confmax, y = ExternalList, yend = ExternalList, color = ItemGroup), position = position_nudge(y = -0.2), size = 3 ) +
  geom_point(data = dataset[ItemGroup == "scCOPs>=5"], aes(x = odds, y = ExternalList, fill = ItemGroup), position = position_nudge(y = -0.2), size = 6, shape = 24) + 
...
g2 = ggplot() +
  geom_bar(data = dataset[ItemGroup == "BulkCOPs"], stat = "identity", aes(x = percOverlap, y = ExternalList, fill = ItemGroup), position = position_nudge(y = 0.15), color = "black",  width = 0.15) +
  geom_bar(data = dataset[ItemGroup == "scCOPs"], stat = "identity", aes(x = percOverlap, y = ExternalList, fill = ItemGroup), position = position_nudge(y = 0.0), color = "black",  width = 0.15) +
  geom_bar(data = dataset[ItemGroup == "scCOPs>=5"], stat = "identity", aes(x = percOverlap, y = ExternalList, fill = ItemGroup), position = position_nudge(y = -0.15), color = "black",  width = 0.15) +
